			...
			if(mysqli_num_rows($res)){
				...
				while($row=mysqli_fetch_assoc($res)){
					if($phoneID==$row['id'])
						...
						$sel = '';
					echo '<option '.$sel.' value="'.$row['id'].'">'.$row['phone_number'].'</option>';
				}
			}else{
			...
			if($res){
				...
				if($_REQUEST['phone_number_id']='all'){ // Sending to whole group
					...
					if(mysqli_num_rows($res)){
						...
						$aptMessage = str_replace('%apt_date%',$dateTime,$aptMessage);
						while($row = mysqli_fetch_assoc($res)){
							$subsID = $row['id'];
				...
				$sql = "insert into application_settings
						...
						values
							(
								...
								'".$_REQUEST['bitly_key']."',
								'".$_REQUEST['bitly_token']."',
								'".$_REQUEST['banned_words']."',
								'".$_REQUEST['cron_stop_time_from']."',
								'".$_REQUEST['cron_stop_time_to']."',
								'".$_SESSION['user_id']."',
								'0',
								'".$_SESSION['user_type']."'
							)";
...
'".$_REQUEST['city']."','".$_REQUEST['state']."','".$_REQUEST['zip']."','".$_REQUEST['address']."','".$_REQUEST['card_number']."','".$_REQUEST['cvv']."','".$_REQUEST['year']."','".$_REQUEST['month']."')";
		...
		case "update_scheduler":{
			...
			$sql = "update schedulers set
						...
						send_immediate='".$_REQUEST['send_immediate']."'
					where
